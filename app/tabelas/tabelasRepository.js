...
/*    SELECT TOP 1000 [PLACA]
  ...
  FROM [FROTA].[dbo].[VEICULO]*/
    ...
    list: function(callback){
        ...
        var listaVeiculos = this.getDAO().list('select * from veiculo',function(error,rs){
          if (error){
              callback(error);
          }
    ...
    listarCombustivel: function(callback){
        var sqlQuery = 'select ID_COMBUSTIVEL as id, DESCRICAO AS descricao from combustivel';
        var listaVeiculos = this.getDAO().list(sqlQuery,function(error,rs){
          if (error){
              callback(error);
          }
          return callback(null,rs);
    ...
    getCombustivelById: function(data,callback){
        var sqlQuery = 'select ID_COMBUSTIVEL as id, DESCRICAO AS descricao from combustivel where ID_COMBUSTIVEL = ' + data.id;

        var listaCombustivel = this.getDAO().list(sqlQuery,function(error,rs){
          if (error){
              callback(error);
          }
          return callback(null,rs);
    ...
    listarCnhsVencidas: function(callback){
       ...
       */
        var sqlQuery = 'SELECT  ' +
                      ...
                      ' FROM ' + this.Motorista().getTableName + '';

        var listaMotoristasCnhVencida = this.getDAO().list(sqlQuery,function(error,rs){
                                                                      if (error){
                                                                          callback(error);
                                                                      }
                                                                      return callback(null,rs);
